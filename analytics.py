...
st.title("📊 Receipt Analytics")
st.markdown("Overview of saved receipt items and full history from `receipt_products`.")

with st.sidebar:
    st.header("🔗 Navigation")
    main_app_url = st.text_input(
        "Main App URL",
        value="http://localhost:8507",
        help="Run `streamlit run app.py --server.port 8507` to start it.",
    )
    st.markdown(f"[Open Main App]({main_app_url})")
...
# Build a transaction key from image_url when available, fallback to store/date/total
